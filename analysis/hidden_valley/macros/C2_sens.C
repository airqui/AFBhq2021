...
std::vector<TH1F*> v_hyield (  std::vector<std::vector<TH1F *>> h1_signal, std::vector<std::vector<TH2F *>> h2_signal,  std::vector<std::vector<TH2F *>> h2_bkg, int step=0) {
  ...
  for (int j = 0; j < h2_signal.size(); j++)
	...
	if(hyield_temp->GetBinCenter(i)<=0 ) {
	  ...
	  if(j==6) hyield_temp->SetBinContent(i,value);
	}
      }
  ...
  for (int isample = 0; isample < sizeof(samples2) / sizeof(TString) ; isample++)
    {
      cout<<isample<<endl;
      //folder=TString::Format("%s/selection_cuts%i",folder_0.Data(),cuts);
      folder=TString::Format("%s/QCDcorrelations_cuts%i",folder_0.Data(),6);
...
void plots_C2Yield_syst_fragmentation(float lum=2000, int yield=0, double factor_det=1, double factor_frag=1) {
 ...
 //PLOTTING
  ...
  for (int j = 0; j < 7; j++)
	...
	y[i-1]= fabs(hyield[j]->GetBinContent(i) -hyield[6]->GetBinContent(i))/ sqrt(pow(hyield[j]->GetBinError(i),2)+pow(hyield[6]->GetBinError(i),2)+pow(0.1*factor_frag*hyield_p->GetBinError(i)*hyield[6]->GetBinError(i),2)  ) ;
  ...
  for(int i=1; i<hyield[6]->GetNbinsX()+1; i++ ) {
    x[i-1]=hyield[6]->GetBinCenter(i);
    y[i-1]= 1;
    ey[i-1]=sqrt(hyield[6]->GetBinError(i)*hyield[6]->GetBinError(i)+hyield[0]->GetBinError(i)*hyield[0]->GetBinError(i));
    n++;
  }
