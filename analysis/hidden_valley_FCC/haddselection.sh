...
do
	...
	hadd -f -j 4 -k results/selection_cuts${cut}_ISR.root output/selection_cuts${cut}_ISR*.root
	...
	hadd -f -j 4 -k results/selectionPL_cuts${cut}_ISR.root output/selectionPL_cuts${cut}_ISR*.root	
done
